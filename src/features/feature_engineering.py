...
# def save_vectorized(vectorizer, save_path: str) -> None:
#     """
#     Save the trained TF-IDF vectorizer.

#     Args:
#         vectorizer: Trained TfidfVectorizer object.
#         save_path (str): File path to save the vectorizer.
#     """
#     try:
...
#         save_vectorized(vectorizer, vectorizer_path)
...
def load_data(file_path: str) -> pd.DataFrame:
    """
    Load CSV data and drop NaN values.
...
def apply_tfidf(train_data: pd.DataFrame, max_features: int, ngram_range: tuple) -> tuple:
    ...
    Returns:
        tuple: TF-IDF features matrix, labels, and vectorizer.
    """
    try:
        vectorizer = TfidfVectorizer(max_features=max_features, ngram_range=ngram_range)
...
def save_features(X, y, save_dir: str) -> None:
    ...
    """
    try:
        processed_data_path = os.path.join(save_dir, 'processed')
        ...
        logger.debug(f'Features and labels saved to {processed_data_path}')
    except Exception as e:
        logger.error(f"Error saving features: {e}")
        raise
